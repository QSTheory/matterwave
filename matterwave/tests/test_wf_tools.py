...
from matterwave.wf_tools import scalar_product, norm
...
def test_scalar_product_norm(space: fa.Space) -> None:
    ...
    sn = scalar_product(arr, arr_conj).values(space)
    n = norm(arr).values(space)
    assert sn == n
...
def test_scalar_product_orth(spaceA: fa.Space, spaceB: fa.Space) -> None:
    ...
    assert np.abs(sp.values("pos")) < 1.e-18
